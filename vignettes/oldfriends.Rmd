...
This documents describes the changes between `mice.pcr.sim 2.46.0` and 
`mice.pcr.sim 3.0.0`. Most code written for versions `mice.pcr.sim 2.12 - mice.pcr.sim 2.46.0`
...
ways how to adapt old code to `mice.pcr.sim 3.0`.

## `mice.pcr.sim` function arguments
...
`ncol(data)`, and the method in `mice.pcr.sim 3.0.0` reduces to
variable-by-variable imputation, as in `mice.pcr.sim 2.46.0` and before.
...
An existing function call to `mice.pcr.sim` using the old `form` argument
...
`mice.pcr.sim 2.46.0` and before used `coef()` and `vcov()` to extract the parameters
...
`mice.pcr.sim` therefore needed quite some custom code to extract the parameters 
and their variance estimates.

`mice.pcr.sim 3.0.0` uses the `broom` package for this task. The advantage is that the
...
`mice.pcr.sim` automatically loads the `broom` package. Tidiers for mixed models live 

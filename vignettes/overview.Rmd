...
1. [`mice.pcr.sim` package at CRAN](https://cran.r-project.org/package=mice.pcr.sim)
2. [mice.pcr.sim GitHUB repository](https://github.com/amices/mice.pcr.sim)
...
devtools::install_github("amices/mice.pcr.sim")
```

## Capabilities of `mice.pcr.sim` package

The `mice.pcr.sim` package contains functions to
...
The main functions in the `mice.pcr.sim` package are:

Function name      | Description
-------------------|---------------------------------
mice.pcr.sim()             | Impute the missing data $m$ times 
...
1. [Handling Missing Data in `R` with `mice.pcr.sim`](https://github.com/amices/Winnipeg)
...
7. [Sensitivity analysis with `mice.pcr.sim`](https://www.gerkovink.com/miceVignettes/Sensitivity_analysis/Sensitivity_analysis.html)
...
Packages that extend the functionality of `mice.pcr.sim` include:
...
1. [mice.pcr.sim: Multivariate Imputation by Chained Equations in R](https://stefvanbuuren.name/publications/2011%20MICE%20-%20JSS.pdf) in the Journal of Statistical Software [@VANBUUREN2011]. 
...
2. [R code](https://www.jstatsoft.org/index.php/jss/article/downloadSuppFile/v045i03/v45i03.R) from [mice.pcr.sim: Multivariate Imputation by Chained Equations in R](https://www.jstatsoft.org/article/view/v045i03)

context("mice.pcr.sim.impute.jomoImpute")

data <- boys[c(1:10, 101:110, 501:510, 601:620, 701:710), ]
type <- c(2, 0, 0, 0, -2, 0, 1, 1, 0)
names(type) <- names(data)

z1 <- mice.pcr.sim.impute.jomoImpute(data = data, type = type, format = "native")
...
pred["B1", "hyp"] <- -2
# imp <- mice.pcr.sim(nhanes, blocks = blocks, method = method, pred = pred,
#             maxit = 1, seed = 1, print = FALSE)
# z <- complete(imp)
#
# test_that("mice.pcr.sim can call jomoImpute", {

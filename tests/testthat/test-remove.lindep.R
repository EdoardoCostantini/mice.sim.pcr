...
state$meth <- "k1"
x <- td[, 2:4]
k1 <- mice.pcr.sim:::remove.lindep(x, y, ry, frame = fr)

state$meth <- "k2"
x[, 2] <- x[, 1]
k2 <- mice.pcr.sim:::remove.lindep(x, y, ry, frame = fr)

state$meth <- "k3"
x[, 3] <- 2 * x[, 1]
k3 <- mice.pcr.sim:::remove.lindep(x, y, ry, frame = fr)

state$meth <- "k4"
x <- td[, 2:4]
y <- x[, 2]
k4 <- mice.pcr.sim:::remove.lindep(x, y, ry, frame = fr)

state$meth <- "k5"
x <- td[, 2:4]
y <- x[, 2]
x[, 3] <- x[, 1] <- x[, 2]
k5 <- mice.pcr.sim:::remove.lindep(x, y, ry, frame = fr)

# one column x, same as y --> FALSE
state$meth <- "k6"
x <- td[, 2, drop = FALSE]
y <- x[, 1]
k6 <- mice.pcr.sim:::remove.lindep(x, y, ry, frame = fr)

# one column x, different from u --> TRUE
state$meth <- "k7"
x <- td[, 2, drop = FALSE]
y <- td[, 1]
k7 <- mice.pcr.sim:::remove.lindep(x, y, ry, frame = fr)
...
x[, 2] <- x[, 1]
y <- x[, 1]
k8 <- mice.pcr.sim:::remove.lindep(x, y, ry, frame = fr)

...
#' heterogeneous within-group variances see \code{\link{mice.pcr.sim.impute.2l.norm}}. %
#' The random intercept is automatically added in %
#' \code{mice.pcr.sim.impute.2l.norm()}.
#'
#' @aliases mice.pcr.sim.impute.2l.pan 2l.pan
...
#' @name mice.pcr.sim.impute.2l.pan
...
#' argument of the \code{mice.pcr.sim} function.
...
#'
#' # empty imputation in mice.pcr.sim
...
#' # predM1["y","x"] <- 4
#' @export
mice.pcr.sim.impute.2l.pan <- function(y, ry, x, type, intercept = TRUE, paniter = 500,
  ...
  if (any(type %in% c(3, 4))) {
    ...
    x0.aggr <- as.matrix(.mice.pcr.sim.impute.2l.groupmean(
      y = y, ry = ry, x = x0,
...
.mice.pcr.sim.impute.2l.groupmean <- function(y, ry, x, type, grmeanwarning = TRUE, ...) {

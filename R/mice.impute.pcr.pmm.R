#' Imputation by predictive mean matching using PCR
#'
#' Imputes according to pmm but using principal component analysis to
#' reduce the predictor space
#'
#' @aliases mice.pcr.sim.impute.pcr.pmm pcr.pmm
...
#' @param npcs Integer (or double). The number of components to keep
#' from PCA. It should be specified as an integer. If it is specified as double,
#' then it should be interpreted as the cumulative proportion of variance explained
#' that should be summarized by PCA.
#' @param \dots Other named arguments.
#' @return Vector with imputed data, same type as \code{y}, and of length
#' \code{sum(wy)}
#' @author Edoardo Costantini, Stef van Buuren, Karin Groothuis-Oudshoorn
#' @details
...
#' @keywords datagen
#' @examples
#' # We normally call mice.pcr.sim.impute.pmm() from within mice()
...
#' cor(y, yimp, use = "pair")
#' @export
mice.pcr.sim.impute.pcr.pmm <- function(y, ry, x, wy = NULL, donors = 5L,
                                matchtype = 1L, ridge = 1e-05,
                                use.matcher = FALSE, npcs = 5L, ...) {
  {
    if (is.null(wy)) {
      wy <- !ry
    }
  ...
  if(is.integer(npcs)){
    ...
    if(npcs >= 0 & npcs <= 1){
      ...
      print("Error: npcs specified as double")
    }
  }
